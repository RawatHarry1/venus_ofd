...
exports.fetchPolygon = async function (req, res) {
  try {
    ...
    cityWrapper = await db.RunQuery(dbConstants.DBS.LIVE_DB, stmt, values);
  ...
  try {
    ...
    var stmnt =
      `UPDATE 
                        ${table} 
                     SET 
                        polygon_coordinates = GeomFromText('POLYGON((` +
      coordinatesString +
      `))') ` +
      `WHERE ${condition}`;

    await db.RunQuery(dbConstants.DBS.LIVE_DB, stmnt, values);
    ...
    var stmnt =
      `UPDATE ${table} SET polygon_coordinates = GeomFromText('POLYGON((` +
      coordinatesString +
      `))') ` +
      `WHERE ${condition}`;
    await db.RunQuery(dbConstants.DBS.AUTH_DB, stmnt, values);

    return responseHandler.success(
      req,
      res,
      'Successfully updated polygon coordinates.',
      '',
    );
  } catch (error) {
    errorHandler.errorHandler(error, req, res);
