...
exports.fetchOprCitiesFields = async function (req, res) {
  try {
    ...
    if (superAdminPanel) {
      ...
      authRequiredFields = editableOprCityFieldsAuthSuperAdmin.filter(
        (field) => !~autosRequiredFields.indexOf(field),
      );
    } else {
      autosRequiredFields = editableOprCityFieldsAutos;

      //fetch only the fields which are not retrieved from autos
      authRequiredFields = editableOprCityFieldsAuth.filter(
        (field) => !~autosRequiredFields.indexOf(field),
      );
    }
    ...
    await Helper.fetchParameterValues(
      operatorId,
    ...
    return responseHandler.success(req, res, '', response.data);
  } catch (error) {
    console.log(error);
...
exports.updateTbOperatorCities = async function (req, res) {
  try {
    ...
    var result = schema.validate({
      cp_operator_id: req.body.cp_operator_id,
    ...
    return responseHandler.success(req, res, 'Successfully updated', '');
  } catch (error) {
    errorHandler.errorHandler(error, req, res);
  }
};
...
exports.addVehicleMake = async function (req, res) {
  try {
    ...
    var schema = Joi.object({
      ...
      vehicle_make_data: Joi.array().items(
        Joi.object().keys({
    ...
    if (req.file) {
      ...
      try {
        var filename =
          Date.now() + '.' + docImage.originalname.split('.').pop();
      } catch (e) {
      ...
      options.vehicle_make_data[0].image =
        wrapperObject && wrapperObject.url ? wrapperObject.url : '';
    }
    ...
    for (var index = 0; index < output.length; index += batchSize) {
      ...
      var placeholders = vehicleDetails
        .map(() => '(?, ?, ?, ?, ?, ?, ?, ?)')
        .join(', ');
...
const editableOprCityFieldsAutosSuperAdmin = [
  'office_address',
  'emergency_no',
  'show_region_specific_fare',
  'driver_support_number',
  'customer_support_number',
  'chat_enabled',
  'advertise_credits',
  'social_links',
  'mandatory_fare_capping_threshold',
  'end_ride_deduct_commission',
  'check_driver_debt',
  'is_gender_enabled',
  'countrywise_cities',
  'county_id',
  'is_driver_preference_enabled',
  'is_enable_driver_multi_cancel_action',
  'number_of_cancellations_allowed',
  'hours_for_allowed_cancellation',
