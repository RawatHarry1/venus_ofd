...
exports.contactCustomers = async function (req, res) {
  try {
    ...
    */
    ...
    let responseLog;
    const stmt = ` SELECT user_id, phone_no, city, country_code FROM ${dbConstants.DBS.LIVE_DB}.${dbConstants.LIVE_DB.CUSTOMERS} WHERE user_id IN (?) AND operator_id = ?`;
    let queryParams = [];
    for (let row of usersData) {
      queryParams.push(row.user_id);
    }
