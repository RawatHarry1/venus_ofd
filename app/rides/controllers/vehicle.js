...
exports.get_city_info_operator_wise = async function (req, res) {
    try {
        ...
        var operatorCityData = await db.RunQuery(dbConstants.DBS.LIVE_DB, fetchCities, values);

        if(requestRideType){
			operatorCityData = operatorCityData.filter(item => {
				const operatorAvailableArray = JSON.parse(item.operator_available).map(String);
      ...
      WHERE
        ...
        var vehicleData = await db.RunQuery(dbConstants.DBS.LIVE_DB, vehicle_query,vehicle_extra_value);
        ...
        for (var i = 0; i < cityData.length; i++) {
            ...
            var str = {
                ...
                polygon_coordinates: operatorCities[cityData[i].city_id].polygon_coordinates,
                from_date: from_date,
                to_date: to_date,
                ...
                elm_verification_enabled: operatorCities[cityData[i].city_id].elm_verification_enabled,
                vehicle_model_enabled: operatorCities[cityData[i].city_id].vehicle_model_enabled,
        ...
        if (fleetId) {
            ...
            var fleetData = await db.RunQuery(dbConstants.DBS.LIVE_DB, fleetNameQuery, values);
...
exports.fetchVehicles = async function (req, res) {
    ...
    var requestRideType = req.request_ride_type
    var requiredVehicleKeys = ['vehicle_type', 'region_id', 'region_name', 'ride_type', 'images'];

    var vehicleCriteria = [{ key: 'operator_id', value: operatorId }, { key: 'city_id', value: cityId }, { key: 'is_active', value: 1 }];

    if(requestRideType ==  rideConstant.CLIENTS.MARS){
        vehicleCriteria.push({
            key: 'ride_type', value: rideConstant.CLIENTS_RIDE_TYPE.MARS
        });
    }else{
        vehicleCriteria.push({
            key: 'ride_type', value: rideConstant.CLIENTS_RIDE_TYPE.VENUS_TAXI
        });
    }
    ...
    let vehicleValues = await db.SelectFromTable(
        ...
        return responseHandler.success(req, res, 'Vehicle list fetched.', vehicleValues);
...
exports.fetchVehicleMake = async function (req, res) {
    try {
        ...
        var vehicleMakeResult = await db.RunQuery(dbConstants.DBS.LIVE_DB, vehicleMakeQuery, values);
        return responseHandler.success(req, res, 'Data Fetched Successfully', vehicleMakeResult);
...
function ensure_prefix_month(date){
	...
	date = date.getFullYear() +"-"+prefix+""+(date.getMonth()+1)+"-"+date.getDate();
	return date;
}